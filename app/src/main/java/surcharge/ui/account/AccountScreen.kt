...
fun AccountScreen(
    ...
    Scaffold(topBar = {
        ...
        Column(
            ...
            if (viewAddDialog) {
                AddArtist(app = app, onDismissRequest = { viewAddDialog = false })
            }
...
private fun SelectArtist(
    ...
    Dialog(onDismissRequest = onDismissRequest) {
        Card {
            ...
            TextButton(
                onClick = {
                    scope.launch { withContext(Dispatchers.IO) { app.settings.updateArtist(selected) } }
                    onDismissRequest()
                },
                modifier = Modifier
                    .padding(end = 10.dp)
    ...
    Dialog(onDismissRequest = onDismissRequest) {
        ElevatedCard {
            ...
            Column(Modifier.fillMaxWidth()) {
                ...
                key(imageUri) {
                    if (imageUri != null && progress.floatValue != 1f) {
                        upload(imageUri!!, name, "display", image, progress)
